...
import { customerAmountDue, formatHkd } from "@/lib/types";
...
export default function PayPage({
  ...
  return (
    <AppShell>
      <PrototypeBanner />
      <AppHeader showBack backHref={`/track/${order.id}`} title="Pay with Airwallex" />
      <main className="mx-auto max-w-[480px] px-4 py-4 pb-28">
        ...
        <p className="mt-4 text-center">
          <Link href={`/track/${order.id}`} className="text-xs font-semibold text-[#ED1C24]">
            Back to tracking
          </Link>
        </p>
      </main>
